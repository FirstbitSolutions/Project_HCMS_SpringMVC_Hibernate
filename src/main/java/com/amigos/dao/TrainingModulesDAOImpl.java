...
public class TrainingModulesDAOImpl implements TrainingModulesDAO {
	...
	public List<TrainingModules> getAll() {
		...
		Transaction tx = null;
		List<TrainingModules> list = new ArrayList<>();
		...
		try {
			...
			list = session.createQuery("FROM TrainingModules", TrainingModules.class).getResultList();
			System.out.println("Retrieve List of Objects : ");
			for (TrainingModules obj : list) {
	...
	public TrainingModules getTrainingModulesById(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			obj = session.get(TrainingModules.class, Integer.valueOf(id.intValue()));
	...
	public TrainingModules update(TrainingModules obj) {
		System.out.println("------------   START --------------");
		Transaction tx = null;
		TrainingModules updatedObj = null;
		Session session = null;
		try {
			session = sessionFactory.openSession();
			System.out.println("Session Created .......");
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			updatedObj = (TrainingModules) session.merge(obj);
	...
	public void delete(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			TrainingModules obj = session.get(TrainingModules.class, Integer.valueOf(id.intValue()));
			if (obj != null) {
				session.remove(obj);
