...
public class ExternalTrainersDAOImpl implements ExternalTrainersDAO {
	...
	public List<ExternalTrainers> getAll() {
		...
		Transaction tx = null;
		List<ExternalTrainers> list = new ArrayList<>();
		...
		try {
			...
			list = session.createQuery("FROM ExternalTrainers", ExternalTrainers.class).getResultList();
			System.out.println("Retrieve List of Objects : ");
			for (ExternalTrainers obj : list) {
	...
	public ExternalTrainers getExternalTrainersById(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			obj = session.get(ExternalTrainers.class, Integer.valueOf(id.intValue()));
	...
	public ExternalTrainers update(ExternalTrainers obj) {
		System.out.println("------------   START --------------");
		Transaction tx = null;
		ExternalTrainers updatedObj = null;
		Session session = null;
		try {
			session = sessionFactory.openSession();
			System.out.println("Session Created .......");
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			updatedObj = (ExternalTrainers) session.merge(obj);
	...
	public void delete(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			ExternalTrainers obj = session.get(ExternalTrainers.class, Integer.valueOf(id.intValue()));
			if (obj != null) {
				session.remove(obj);
