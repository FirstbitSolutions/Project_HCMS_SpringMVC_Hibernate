...
public class LeaveApprovalsDAOImpl implements LeaveApprovalsDAO {
	...
	public List<LeaveApprovals> getAll() {
		...
		Transaction tx = null;
		List<LeaveApprovals> list = new ArrayList<>();
		...
		try {
			...
			list = session.createQuery("FROM LeaveApprovals", LeaveApprovals.class).getResultList();
			System.out.println("Retrieve List of Objects : ");
			for (LeaveApprovals obj : list) {
	...
	public LeaveApprovals getLeaveApprovalsById(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			obj = session.get(LeaveApprovals.class, Integer.valueOf(id.intValue()));
	...
	public LeaveApprovals update(LeaveApprovals obj) {
		System.out.println("------------   START --------------");
		Transaction tx = null;
		LeaveApprovals updatedObj = null;
		Session session = null;
		try {
			session = sessionFactory.openSession();
			System.out.println("Session Created .......");
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			updatedObj = (LeaveApprovals) session.merge(obj);
	...
	public void delete(Long id) {
		...
		try {
			...
			tx = session.beginTransaction();
			System.out.println("Transaction Started .......");
			LeaveApprovals obj = session.get(LeaveApprovals.class, Integer.valueOf(id.intValue()));
			if (obj != null) {
				session.remove(obj);
